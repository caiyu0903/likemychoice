(function ($) {
   ...
   */
    ...
    class CustomSelect extends ccComponent {
        constructor() {
            var name = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'custom-select';
            var cssSelector = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : ".cc-select";
            super(name, cssSelector);
        }
    ...
    theme.VideoManager = new function () {
        ...
        _._loadYoutubeVideos = function (container) {
            $('.video-container[data-video-type="youtube"]:not(.video--init)', container).each(function () {
                ...
                fetch('https://www.youtube.com/oembed?format=json&url=' + encodeURIComponent($(this).data('video-url'))).then((response) => {
                    if (!response.ok) {
        ...
        _._loadVimeoVideos = function (container) {
            $('.video-container[data-video-type="vimeo"]:not(.video--init)', container).each(function () {
                ...
                fetch('https://vimeo.com/api/oembed.json?url=' + encodeURIComponent($(this).data('video-url'))).then((response) => {
                    if (!response.ok) {
                        throw new Error("HTTP error! Status: ".concat(response.status));
                    }
                    return response.json();
            ...
            if (!_.pageshowListenerAdded) {
                ...
                window.addEventListener('pageshow', (event) => {
                    if (event.persisted) {
                        ...
                        Object.keys(_.videos.videoData).filter((key) => _.videos.videoData[key].type === 'yt' && _.videos.videoData[key].isBackgroundVideo).forEach((key) => {
    ...
    theme.MapSection = new function () {
        ...
        this.createMap = function (container) {
            ...
            return _.geolocate($map).then(
                ...
                if (Shopify.designMode) {
                    ...
                    $mapContainer.find('.map-section__wrapper').html(
        ...
        this.onSectionLoad = function (target) {
            ...
            window.gm_authFailure = function () {
                ...
                $container.find('.map-section__wrapper').html(
    ...
    theme.validateProductForm = function ($formContainer) {
        $formContainer.find('.option-selector--empty').removeClass('option-selector--empty').find('.label__prefix').remove();
    ...
    theme.addDelegateEventListener = function (element, eventName, selector, callback) {
        var addEventListenerParams = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
        var cb = (evt) => {
    ...
    var CartForm = class extends HTMLElement {
        ...
        refresh(openDrawer) {
            ...
            fetch("".concat(window.Shopify.routes.root, "?section_id=").concat(this.sectionId)).then((response) => {
                if (!response.ok) {
                    throw new Error("HTTP error! Status: ".concat(response.status));
                }
    ...
    var CCCartCrossSell = class extends HTMLElement {
        ...
        init() {
            ...
            if (this.dataset.from) {
                fetch(this.dataset.from).then((response) => {
                    if (!response.ok) {
                        throw new Error("HTTP error! Status: ".concat(response.status));
                    }
                    return response.text();
   ...
   */
    ...
    theme.SlideshowSection = new function () {
        ...
        this.onBlockSelect = function (container) {
            $(container).closest('.slick-slider').slick('slickGoTo', $(container).data('slick-index')).slick('slickPause');
        };
        ...
        $(document).on('load-product-form', '.product-form', function (e) {
            ...
            if ($availabilityDependentOptions.length) {
                $(this).on('variantChanged', function (evt, variant) {
                    ...
                    for (var _optionIndex = 0; _optionIndex < productData.options.length; _optionIndex++) {
                        _loop4(_optionIndex);
                    }
